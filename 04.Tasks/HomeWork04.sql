...
-- Решение 2 - поиск самого болтливого юзера без заданного айдишника
 SELECT id, CONCAT(firstname, ' ', lastname) AS 'Spamer' FROM users WHERE id = (SELECT from_user_id FROM messages GROUP BY from_user_id, to_user_id ORDER BY COUNT(*) DESC LIMIT 1); 
